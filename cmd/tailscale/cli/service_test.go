...
func TestServiceListTable(t *testing.T) {
	...
	// Header. The NAME column was dropped; DNS NAME leads, ACTIONS trails.
	for _, h := range []string{"DNS NAME", "DISPLAY NAME", "IP", "ENDPOINTS", "ACTIONS"} {
		if !strings.Contains(out, h) {
			t.Errorf("output missing header %q\n%s", h, out)
		}
	}
	if strings.Contains(out, "NAME\t") && !strings.Contains(out, "DNS NAME") {
		t.Errorf("NAME column should have been dropped\n%s", out)
	}
	...
	if !strings.Contains(out, "postgresql(?)") {
		t.Errorf("output missing svc:db inferred action %q\n%s", "postgresql(?)", out)
	}
}

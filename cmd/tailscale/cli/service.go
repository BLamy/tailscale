...
func runServiceList(ctx context.Context, args []string) error {
	...
	defer w.Flush()
	fmt.Fprintf(w, "\n %s\t%s\t%s\t%s\t%s\t", "DNS NAME", "DISPLAY NAME", "IP", "ENDPOINTS", "ACTIONS")
...
// ACTIONS column, comma-separated: "-" for none, the single type for one, and
// a comma-separated list for more.
//
// Explicit actions are shown by type. When a Service carries no explicit
// actions, actions are inferred from well-known TCP ports (see
// [wellKnownPortActions]) and marked with a "(?)" suffix.
func serviceActionTypes(svc tailcfg.ServiceDetails) string {
	if len(svc.Actions) > 0 {
		types := make([]string, len(svc.Actions))
	...
	for _, ppr := range svc.Ports {
		...
		t, ok := wellKnownPortActions[ppr.Ports.First]
		if !ok || seen[t] {
			continue
		}
		seen[t] = true
		inferred = append(inferred, string(t)+"(?)")
	}
	if len(inferred) == 0 {
		return "-"
	}
